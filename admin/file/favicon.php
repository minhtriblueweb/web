...
<!-- Main content -->
<?php
$get_favicon = $setting->get_setting_item("favicon");
if ($get_favicon) {
  $result_favicon = $get_favicon->fetch_assoc();
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload'])) {
  $update = $setting->update_setting_item("favicon", $_POST, $_FILES);
}
?>
          ...
          <div class="upload-file">
            ...
            <label class="upload-file-label mb-2" for="file">
              <div class="upload-file-image rounded mb-3">
                <img
                  src="<?= empty($result_favicon['favicon']) ? $config['baseAdmin'] . "assets/img/noimage.png" : $config['baseAdmin'] . "uploads/" . htmlspecialchars($result_favicon['favicon']); ?>"
                  class="rounded img-upload" id="preview-image">
              </div>
              <div class="custom-file my-custom-file">
                <input type="file" class="custom-file-input" name="favicon" id="file" lang="vi">
                <label class="custom-file-label mb-0" data-browse="Chọn" for="file">Chọn file</label>
              </div>
            </label>
            <strong class="d-block text-sm">Width: 48 px - Height: 48 px
