...
<!-- Main content -->
<?php
$show_phuongthuctt = $phuongthuctt->show_phuongthuctt();
if (isset($_GET['del'])) {
  $id = $_GET['del'];
  $del = $phuongthuctt->del_phuongthuctt($id);
}
if (isset($_GET['listid'])) {
  $listid = $_GET['listid'];
  $xoanhieu = $phuongthuctt->xoanhieu_phuongthuctt($listid);
}
?>
...
<section class="content">
  <div class="card-footer text-sm sticky-top">
    <a class="btn btn-sm bg-gradient-primary text-white" href="themphuongthuctt.php" title="Thêm mới"><i
    ...
    <div class="card-body table-responsive p-0">
      <table class="table table-hover">
        ...
        <tbody>
          ...
          ?>
            <?php while ($resule = $show_phuongthuctt->fetch_assoc()):
            ?>
              <tr>
                ...
                <td class="align-middle">
                  <input type="number" class="form-control form-control-mini m-auto update-numb" min="0"
                    value="<?= $resule['numb']; ?>" data-id="<?= $resule['id']; ?>" data-table="tbl_phuongthuctt">
                </td>
                <td class="align-middle">
                  <a href="themphuongthuctt.php?id=<?= $resule['id']; ?>" title="<?= $resule['name']; ?>">
                    <img
                      src='<?php echo $config['baseAdmin'] . (empty($resule['file']) ? "assets/img/noimage.png" : "uploads/" . $resule['file']); ?>'
                      alt='<?= $resule['name']; ?>' class='rounded img-preview' /> </a>
                </td>
                <td class="align-middle">
                  <a class="text-dark text-break" href="themphuongthuctt.php?id=<?= $resule['id']; ?>"
                    title="<?= $resule['name']; ?>"><?= $resule['name']; ?></a>
                </td>
                <td class="align-middle text-center">
                  <div class="custom-control custom-checkbox my-checkbox">
                    <input type="checkbox" class="custom-control-input show-checkbox"
                      id="show-checkbox-hienthi-<?= $resule['id']; ?>" data-table="tbl_phuongthuctt" data-type="hienthi"
                      data-id="<?= $resule['id']; ?>"
                ...
                <td class="align-middle text-center text-md text-nowrap">
                  <a class="text-primary mr-2" href="themphuongthuctt.php?id=<?= $resule['id']; ?>" title="Chỉnh sửa"><i
