...
<!-- Main content -->
<?php
$get_huongdan = $trangtinh->get_static('huongdan');
if ($get_huongdan) {
  $result_huongdan = $get_huongdan->fetch_assoc();
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload'])) {
  $update = $trangtinh->update_static($_POST, 'huongdan', $result_huongdan['id']);
}
?>
  ...
  <form class="validation-form" novalidate="" method="post" id="form-watermark" action="" enctype="multipart/form-data">
    ...
    <div class="card card-primary card-outline text-sm">
      <div class="card-header">
        <h3 class="card-title">Nội dung Hướng dẫn</h3>
              ...
              <div class="tab-pane fade show active" id="tabs-lang-vi" role="tabpanel" aria-labelledby="tabs-lang">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control for-seo text-sm" name="namevi" id="namevi"
                    placeholder="Tiêu đề (vi)" value="<?= $result_huongdan['namevi'] ?>" required>
                </div>
                <div class="form-group">
                  ...
                  <textarea class="form-control for-seo text-sm form-control-ckeditor" name="contentvi" id="contentvi"
                    placeholder="Nội dung (vi)"><?= $result_huongdan['contentvi'] ?></textarea>
                </div>
              </div>
            </div>
          </div>
                ...
                <div class="tab-pane fade show active" id="tabs-seolang-vi" role="tabpanel" aria-labelledby="tabs-lang">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control check-seo title-seo text-sm" name="titlevi" id="titlevi"
                      placeholder="SEO Title (vi)" value="<?= $result_huongdan['titlevi'] ?>">
                  </div>
                  <div class="form-group">
                    <div class="label-seo">
                    ...
                    <input type="text" class="form-control check-seo keywords-seo text-sm" name="keywordsvi"
                      id="keywordsvi" placeholder="SEO Keywords (vi)" value="<?= $result_huongdan['keywordsvi'] ?>">
                  </div>
                  <div class="form-group">
                    <div class="label-seo">
                    ...
                    <textarea class="form-control check-seo description-seo text-sm" name="descriptionvi"
                      ...
                      placeholder="SEO Description (vi)"><?= $result_huongdan['descriptionvi'] ?></textarea>
                  </div>
                </div>
              </div>
            </div>
    ...
    <input type="hidden" name="type" id="type" value="huongdan">
  </form>
</section>
<!-- Main Footer -->
