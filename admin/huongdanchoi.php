...
if (isset($_GET['del'])) {
  $id = $_GET['del'];
  $del = $news->del_news($id, 'huongdanchoi');
}
if (isset($_GET['listid'])) {
  $listid = $_GET['listid'];
  $xoanhieu = $news->delete_multiple_news($listid, 'huongdanchoi');
}
$records_per_page = 10; // Số bản ghi trên mỗi trang
$current_page = isset($_GET['page']) ? (int)$_GET['page'] : 1; // Trang hiện tại
$total_records = $functions->phantrang('tbl_news', 'huongdanchoi'); // Lấy tổng số bản ghi
$total_pages = ceil($total_records / $records_per_page); // Tính số trang
// Lấy dữ liệu cho trang hiện tại
$show_huongdanchoi = $news->show_news($records_per_page, $current_page, '', 'huongdanchoi');
?>
  ...
  <div class="container-fluid">
    <div class="row">
      <ol class="breadcrumb float-sm-left">
        ...
        <li class="breadcrumb-item">hướng dẫn chơi</li>
      </ol>
    </div>
  </div>
</section>
<section class="content">
  <div class="card-footer text-sm sticky-top">
    <a class="btn btn-sm bg-gradient-primary text-white" href="themhuongdanchoi.php" title="Thêm mới"><i
  ...
  <div class="card card-primary card-outline text-sm mb-0">
    <div class="card-header">
      <h3 class="card-title">Danh sách hướng dẫn chơi</h3>
    </div>
      ...
      <table class="table table-hover">
        ...
        <form action="" method="POST">
          <tbody>
            ...
            <?php while ($resule = $show_huongdanchoi->fetch_assoc()): ?>
            <tr>
              ...
              <td class="align-middle">
                <a href="suahuongdanchoi.php?id=<?= $resule['id'] ?>" title="<?= $resule['namevi'] ?>">
              ...
              <td class="align-middle">
                <a class="text-dark text-break" href="suahuongdanchoi.php?id=<?= $resule['id'] ?>"
                  title="<?= $resule['namevi'] ?>"><?= $resule['namevi'] ?></a>
                <div class="tool-action mt-2 w-clear">
                  <a class="text-primary mr-3" href="<?= "{$config['base']}{$resule['slugvi']}" ?>"
                    target="_blank" title="<?= $resule['namevi'] ?>"><i class="far fa-eye mr-1"></i>View</a>
                  <a class="text-info mr-3" href="suahuongdanchoi.php?id=<?= $resule['id'] ?>"
              ...
              <td class="align-middle text-center text-md text-nowrap">
                <a class="text-primary mr-2" href="suahuongdanchoi.php?id=<?= $resule['id'] ?>" title="Chỉnh sửa"><i
