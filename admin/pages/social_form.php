...
$message = '';
$name_page = 'social';
$table = "tbl_{$name_page}";
$linkMan = "index.php?page={$name_page}_list";
$thumb_width = '30';
$thumb_height = '30';
$result = [];
$id = $_GET['id'] ?? null;
                  ...
                  <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                    <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>"
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control for-seo text-sm" name="desc<?= $k ?>" id="desc<?= $k ?>" placeholder="Mô tả"
                          value="<?= $_POST['desc' . $k] ?? ($result['desc' . $k] ?? '') ?>" />
                      </div>
                    </div>
                  <?php } ?>
                </div>
