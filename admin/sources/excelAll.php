...
if ($is_logged_in == false && $act != "login") {
...
/* Setting */
$setting = $d->rawQueryOne("select * from tbl_setting limit 0,1");
$optsetting = (isset($setting['options']) && $setting['options'] != '') ? json_decode($setting['options'], true) : null;

/* Thông tin đơn hàng */
$sql = "select * from tbl_order where id<>0";
...
$PHPExcel->setActiveSheetIndex(0)->setCellValue('A4', diachi.': ' . $setting['addressvi']);
...
for ($i = 0; $i < count($orders); $i++) {
    ...
    /* Trang thái */
    $order_status_info = $d->rawQueryOne("select namevi from tbl_order_status where id = ? limit 0,1", array($orders[$i]['order_status']));
