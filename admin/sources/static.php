...
{
  global $d, $item, $type;
  $item = $d->rawQueryOne("select * from tbl_static where type = ? limit 0,1", array($type));
}
...
function saveStatic()
{
  ...
  $message = '';
  $response = array();
  $static = $d->rawQueryOne("select * from tbl_static where type = ? limit 0,1", array($type));
  ...
  if (!empty($static)) {
    ...
    if ($d->update('static', $data)) {
      ...
      if ($func->hasFile("file")) {
        ...
        if ($photo = $func->uploadImage("file", $config['static'][$type]['img_type'], UPLOAD, $file_name)) {
          $row = $d->rawQueryOne("select id, photo from tbl_static where type = ? limit 0,1", array($type));

          if (!empty($row)) {
            $func->deleteFile(UPLOAD . $row['photo']);
          }
      ...
      if ($func->hasFile("video")) {
        ...
        if ($video = $func->uploadImage("video", $config['static'][$type]['video_type'], UPLOAD, $file_name)) {
          $row = $d->rawQueryOne("select id, video from tbl_static where type = ? limit 0,1", array($type));
          if (!empty($row)) {
            $func->deleteFile(UPLOAD . $row['video']);
          }
      ...
      if ($func->hasFile("file1")) {
        ...
        if ($photo1 = $func->uploadImage("file1", $config['static'][$type]['img_type'], UPLOAD, $file_name1)) {
          $row1 = $d->rawQueryOne("select id, photo1 from tbl_static where type = ? limit 0,1", array($type));

          if (!empty($row1)) {
            $func->deleteFile(UPLOAD . $row1['photo1']);
          }
      ...
      if ($func->hasFile("file_attach")) {
        ...
        if ($file_attach = $func->uploadImage("file_attach", $config['static'][$type]['file_type'], UPLOAD, $file_name)) {
          $row = $d->rawQueryOne("select id, file_attach from tbl_static where type = ? limit 0,1", array($type));

          if (!empty($row)) {
            $func->deleteFile(UPLOAD . $row['file_attach']);
          }
      ...
      /* SEO */
      if (isset($config['static'][$type]['seo']) && $config['static'][$type]['seo'] == true) {
        $d->rawQuery("delete from tbl_seo where com = ? and act = ? and type = ?", array($com, 'update', $type));
