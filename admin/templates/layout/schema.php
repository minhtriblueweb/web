        ...
        <?php foreach ($config['website']['seo'] as $k => $v) { ?>
          <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-schemalang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
            <div class="form-group">
              ...
              <textarea class="form-control schema-seo" name="dataSchema[schema<?= $k ?>]" id="schema<?= $k ?>" rows="15" placeholder="<?= neuquykhachkhongbietcachsudungdatastructurevuilongkhongnhapnoidungvaokhungnaydetranhphatsinhloi ?>"><?= @$seoDB['schema' . $k] ?></textarea>
            </div>
          </div>
        <?php } ?>
      </div>
