...
$linkAdd = "index.php?com=product&act=add_brand&type=" . $type;
$linkEdit = "index.php?com=product&act=edit_brand&type=" . $type;
...
<section class="content">
  ...
  <div class="card card-primary card-outline text-sm mb-0">
    <div class="card-header">
      <h3 class="card-title"><?= danhsach ?> <?= $config['product'][$type]['title_main_brand'] ?></h3>
    </div>
          ...
          <tbody>
            <?php for ($i = 0; $i < count($items); $i++) { ?>
              <tr>
                ...
                <td class="align-middle">
                  <input type="number" class="form-control form-control-mini m-auto update-numb" min="0" value="<?= $items[$i]['numb'] ?>" data-id="<?= $items[$i]['id'] ?>" data-table="product_brand">
                </td>
                <?php if (isset($config['product'][$type]['show_images_brand']) && $config['product'][$type]['show_images_brand'] == true) { ?>
                  <td class="align-middle">
                    <a href="<?= $linkEdit ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['namevi'] ?>">
                      <?= $func->getImage(['class' => 'rounded img-preview', 'sizes' => $config['product'][$type]['thumb_brand'], 'upload' => UPLOAD_PRODUCT_L, 'image' => $items[$i]['photo'], 'alt' => $items[$i]['namevi']]) ?>
                    </a>
                  </td>
                <?php } ?>
                <td class="align-middle">
                  ...
                  foreach ($config['product'][$type]['check_brand'] as $key => $value) { ?>
                    <td class="align-middle text-center">
                      <label class="switch switch-success">
                        <input type="checkbox" class="switch-input custom-control-input show-checkbox" id="show-checkbox-<?= $key ?>-<?= $items[$i]['id'] ?>" data-table="product_brand" data-id="<?= $items[$i]['id'] ?>" data-attr="<?= $key ?>" <?= (in_array($key, $status_array)) ? 'checked' : '' ?>>
                      </label>
                    </td>
                <?php }
                } ?>
