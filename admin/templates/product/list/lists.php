...
$linkAdd = "index.php?com=product&act=add_list&type=" . $type;
$linkEdit = "index.php?com=product&act=edit_list&type=" . $type;
...
<section class="content">
  ...
  <div class="card card-primary card-outline text-sm mb-0">
    <div class="card-header">
      <h3 class="card-title"><?= danhsach ?> <?= $config['product'][$type]['title_main_list'] ?></h3>
    </div>
                ...
                <?php if (isset($config['product'][$type]['show_images_list']) && $config['product'][$type]['show_images_list'] == true) { ?>
                  <td class="align-middle">
                    <a href="<?= $linkEdit ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['namevi'] ?>">
                      <?= $func->getImage(['class' => 'rounded img-preview', 'sizes' => $config['product'][$type]['thumb_list'], 'upload' => UPLOAD_PRODUCT_L, 'image' => $items[$i]['photo'], 'alt' => $items[$i]['namevi']]) ?>
                    </a>
                  </td>
                <?php } ?>
                <td class="align-middle">
                  ...
                  foreach ($config['product'][$type]['check_list'] as $key => $value) { ?>
                    <td class="align-middle text-center">
                      <label class="switch switch-success">
                        <input type="checkbox" class="switch-input custom-control-input show-checkbox" id="show-checkbox-<?= $key ?>-<?= $items[$i]['id'] ?>" data-table="product_list" data-id="<?= $items[$i]['id'] ?>" data-attr="<?= $key ?>" <?= (in_array($key, $status_array)) ? 'checked' : '' ?>>
                      </label>
                    </td>
                <?php }
                } ?>
