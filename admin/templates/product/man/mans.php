  ...
  <?php if (
    ...
    <div class="card-footer form-group-category text-sm bg-light row">
      ...
      <?php if (isset($config['product'][$type]['vari']) && $config['product'][$type]['vari'] == true) { ?>
        <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?= $func->getLinkCategory('product', 'vari', $type) ?></div>
      <?php } ?>
      <?php if (isset($config['product'][$type]['brand']) && $config['product'][$type]['brand'] == true) { ?>
        <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?= $func->getLinkCategory('product', 'brand', $type, 'Hãng sản phẩm') ?></div>
      <?php } ?>
    </div>
  <?php } ?>
        ...
        <?php } else { ?>
          <tbody>
            <?php for ($i = 0; $i < count($items); $i++) {
              ...
              // if ($items[$i]['id_vari']) $linkID .= "&id_vari=" . $items[$i]['id_vari'];
                  ...
                  foreach ($config['product'][$type]['check'] as $key => $value) { ?>
                    <td class="align-middle text-center">
                      <label class="switch switch-success">
                        <input type="checkbox" class="switch-input custom-control-input show-checkbox" id="show-checkbox-<?= $key ?>-<?= $items[$i]['id'] ?>" data-table="product" data-id="<?= $items[$i]['id'] ?>" data-attr="<?= $key ?>" <?= (in_array($key, $status_array)) ? 'checked' : '' ?>>
                      </label>
                    </td>
                <?php }
                } ?>
