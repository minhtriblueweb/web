  ...
  <div class="container-fluid">
    <div class="row">
      <ol class="breadcrumb float-sm-left">
        ...
        <li class="breadcrumb-item active"><?= ($id > 0 ? capnhat : themmoi) . ' ' . ($config['product'][$type]['title_main_sub'] ?? '') ?></li>
      </ol>
    </div>
  </div>
</section>
  ...
  <form class="validation-form" novalidate method="post" action="" enctype="multipart/form-data">
    <div class="card-footer text-sm sticky-top">
      <button type="submit" name="<?= !empty($id) ? 'edit' : 'add'; ?>"
        class="btn btn-sm bg-gradient-primary <?= !empty($config['product'][$type]['slug_sub']) ? 'submit-check' : '' ?>"
        <?= !empty($config['product'][$type]['slug_sub']) ? 'disabled' : '' ?>>
        <i class="far fa-save mr-2"></i><?= luu ?>
      </button>
      <button type="submit" class="btn btn-sm bg-gradient-success <?= !empty($config['product'][$type]['slug_sub']) ? 'submit-check' : '' ?>" name="save-here"><i class="far fa-save mr-2"></i><?= luutaitrang ?></button>
    ...
    <div class="row">
      <div class="col-xl-8">
        <?php if (!empty($config['product'][$type]['slug_sub'])): ?>
                ...
                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                  <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                    <div class="tab-pane fade show <?= ($k == $lang) ? 'active' : '' ?>"
                      ...
                      <!-- Mô tả -->
                      <?php if (!empty($config['product'][$type]['desc_cke_sub']) || !empty($config['product'][$type]['desc_sub'])): ?>
                        <div class="form-group">
                          <label for="desc<?= $k ?>"><?= mota ?> (<?= $k ?>):</label>
                          <textarea rows="4" class="form-control for-seo text-sm <?= !empty($config['product'][$type]['desc_cke_sub']) ? 'form-control-ckeditor' : '' ?>"
                      ...
                      <!-- Nội dung -->
                      <?php if (!empty($config['product'][$type]['content_cke_sub']) || !empty($config['product'][$type]['content_sub'])): ?>
                        <div class="form-group">
                          <label for="content<?= $k ?>"><?= mota ?> (<?= $k ?>):</label>
                          <textarea rows="4" class="form-control for-seo text-sm <?= !empty($config['product'][$type]['content_cke_sub']) ? 'form-control-ckeditor' : '' ?>"
      ...
      <div class="col-xl-4">
        ...
        <?php if (!empty($config['product'][$type]['images_sub'])): ?>
          <div class="card card-primary card-outline text-sm">
            <div class="card-header">
        ...
        <div class="card card-primary card-outline text-sm">
          ...
          <div class="card-body">
            <div class="form-group">
              <?php foreach ($config['product'][$type]['check_sub'] as $check => $label): ?>
