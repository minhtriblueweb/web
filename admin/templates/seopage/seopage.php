        ...
        <div class="card-seo">
          <div class="card card-primary card-outline card-outline-tabs">
            <div class="card-header p-0 border-bottom-0">
              <ul class="nav nav-tabs" id="custom-tabs-lang" role="tablist"><?php foreach ($config['website']['lang'] as $k => $v): ?><li class="nav-item"><a class="nav-link <?= ($k == $lang) ? 'active' : '' ?>" id="tab-<?= $k ?>" data-toggle="pill" href="#tab-content-<?= $k ?>" role="tab" aria-controls="tab-content-<?= $k ?>" aria-selected="<?= ($k == $lang) ? 'true' : 'false' ?>">SEO (<?= $v ?>)</a></li><?php endforeach; ?></ul>
            </div>
            <div class="card-body">
              <div class="tab-content" id="tab-content-lang"><?php foreach ($config['website']['lang'] as $k => $v): ?><div class="tab-pane fade show <?= ($k == $lang) ? 'active' : '' ?>" id="tab-content-<?= $k ?>" role="tabpanel" aria-labelledby="tab-<?= $k ?>">
                    <div class="form-group">
                      <div class="label-seo"><label for="title<?= $k ?>">SEO Title (<?= $k ?>):</label><strong class="count-seo"><span>0</span>/70 <?= kytu ?></strong></div><input type="text" class="form-control check-seo title-seo text-sm" name="data[title<?= $k ?>]" id="title<?= $k ?>" placeholder="SEO Title (<?= $k ?>)" value="<?= $_POST['title' . $k] ?? ($seo_data['title' . $k] ?? '') ?>">
                    </div>
                    <div class="form-group">
                      <div class="label-seo"><label for="keywords<?= $k ?>">SEO Keywords (<?= $k ?>):</label><strong class="count-seo"><span>0</span>/70 <?= kytu ?></strong></div><input type="text" class="form-control check-seo keywords-seo text-sm" name="data[keywords<?= $k ?>]" id="keywords<?= $k ?>" placeholder="SEO Keywords (<?= $k ?>)" value="<?= $_POST['keywords' . $k] ?? ($seo_data['keywords' . $k] ?? '') ?>">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="seo-preview-google">
                      <div class="d-flex align-items-start mb-1"><?php if (!empty($favicon['file'])): ?><div class="favicon-wrapper mr-2 rounded-circle"><?= $fn->getImage(['file' => $favicon['file'], 'class' => 'favicon rounded-circle border', 'alt' => 'favicon', 'title' => 'favicon']) ?></div><?php else: ?><div class="favicon-wrapper mr-2 rounded-circle" style="background-color:#e5edff;"><svg style="width:20px;height:20px;" focusable="false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                          ...
                          <div class="seo-url text-muted small" id="seourlpreview<?= $k ?>"><?= rtrim(BASE, '/') ?> › <?= !empty($_POST['slug' . $k]) ? $_POST['slug' . $k] : ($result['slug' . $k] ?? '') ?><span class="seo-dot-menu ml-1">⋮</span></div>
                        </div>
                      </div>
                      <div class="seo-title" id="title-seo-preview<?= $k ?>"><?= trim($_POST['title' . $k] ?? $seo_data['title' . $k] ?? '') ?: 'Tiêu đề mô phỏng trang website của bạn' ?></div>
                      <div class="seo-description" id="description-seo-preview<?= $k ?>"><?= trim($_POST['description' . $k] ?? $seo_data['description' . $k] ?? '') ?: 'Mô tả ngắn gọn sẽ hiển thị ở đây, giúp người dùng hiểu nội dung trang. Giữ khoảng 150-160 ký tự là đẹp.' ?></div>
                    </div>
                  </div><input type="hidden" id="seo-create" value="<?= implode(',', array_keys($config['website']['lang'])) ?>"><?php endforeach; ?></div>
            </div>
          </div>
        </div>
      </div>
