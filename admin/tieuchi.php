...
<!-- Main content -->
<?php
$show_tieuchi = $tieuchi->show_tieuchi();
if (isset($_GET['del'])) {
  $id = $_GET['del'];
  $del = $tieuchi->del_tieuchi($id);
}
if (isset($_GET['listid'])) {
  $listid = $_GET['listid'];
  $xoanhieu = $tieuchi->xoanhieu_tieuchi($listid);
}
?>
  ...
  <div class="container-fluid">
    <div class="row">
      <ol class="breadcrumb float-sm-left">
        ...
        <li class="breadcrumb-item active">Tiêu chí</li>
      </ol>
    </div>
  </div>
</section>
...
<section class="content">
  <div class="card-footer text-sm sticky-top">
    <a class="btn btn-sm bg-gradient-primary text-white" href="themtieuchi.php" title="Thêm mới"><i
              ...
              <tr>
                ...
                <td class="align-middle">
                  <input type="number" class="form-control form-control-mini m-auto update-numb" min="0"
                    value="<?= $resule['numb']; ?>" data-id="<?= $resule['id']; ?>" data-table="tbl_tieuchi">
                </td>
                <td class="align-middle">
                  <a href="themtieuchi.php?id=<?= $resule['id']; ?>" title="<?= $resule['name']; ?>">
                    <img
                ...
                <td class="align-middle">
                  <a class="text-dark text-break" href="themtieuchi.php?id=<?= $resule['id']; ?>"
                    title="<?= $resule['name']; ?>"><?= $resule['name']; ?></a>
                </td>
                <td class="align-middle text-center">
                  <div class="custom-control custom-checkbox my-checkbox">
                    <input type="checkbox" class="custom-control-input show-checkbox"
                      id="show-checkbox-hienthi-<?= $resule['id']; ?>" data-table="tbl_tieuchi" data-type="hienthi"
                      data-id="<?= $resule['id']; ?>"
                ...
                <td class="align-middle text-center text-md text-nowrap">
                  <a class="text-primary mr-2" href="themtieuchi.php?id=<?= $resule['id']; ?>" title="Chỉnh sửa"><i
