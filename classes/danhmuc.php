...
class danhmuc
{
    ...
    {
        $query = "SELECT * FROM tbl_danhmuc ORDER BY numb,id DESC";
        $result = $this->db->select($query);
        return $result;
    }
    ...
    public function show_danhmuc_pt($records_per_page, $current_page)
    {
        ...
        $query = "SELECT * FROM tbl_danhmuc ORDER BY numb,id DESC LIMIT $records_per_page OFFSET $offset";
        $result = $this->db->select($query);
        return $result;
    }
    ...
    public function update_danhmuc($data, $files, $id)
    {
        ...
        if (!empty($file_name)) {
            ...
            if ($old_file && $old_file->num_rows > 0) {
                ...
                if (file_exists($old_file_path) && !empty($row['file_name'])) {
        ...
        if (!empty($unique_image)) {
            $update_fields[] = "file_name = '$unique_image'";
        }
