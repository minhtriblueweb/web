<?php
/* Giới thiệu */
$nametype = "gioithieu";
$config['static'][$nametype]['title_main'] = "Giới thiệu";
$config['static'][$nametype]['check'] = array("hienthi" => "Hiển thị");
$config['static'][$nametype]['name'] = true;
$config['static'][$nametype]['content_cke'] = true;

/* Liên hệ */
$nametype = "lienhe";
$config['static'][$nametype]['title_main'] = "Liên hệ";
$config['static'][$nametype]['check'] = array("hienthi" => "Hiển thị");
$config['static'][$nametype]['name'] = true;
$config['static'][$nametype]['content_cke'] = true;

/* Footer */
$nametype = "footer";
$config['static'][$nametype]['title_main'] = "Footer";
$config['static'][$nametype]['check'] = array("hienthi" => "Hiển thị");
$config['static'][$nametype]['name'] = true;
$config['static'][$nametype]['content_cke'] = true;

/* Mua hàng */
$nametype = "muahang";
$config['static'][$nametype]['title_main'] = "Mua hàng";
$config['static'][$nametype]['check'] = array("hienthi" => "Hiển thị");
$config['static'][$nametype]['name'] = true;
$config['static'][$nametype]['content_cke'] = true;

/* Hỗ trợ khách hàng */
$nametype = "hotrokhachhang";
$config['static'][$nametype]['title_main'] = "Hỗ trợ khách hàng";
$config['static'][$nametype]['check'] = array("hienthi" => "Hiển thị");
$config['static'][$nametype]['name'] = true;
$config['static'][$nametype]['content_cke'] = true;

/* Hỗ trợ 24/7 */
$nametype = "hotro247";
$config['static'][$nametype]['title_main'] = "Hỗ trợ 24/7";
$config['static'][$nametype]['check'] = array("hienthi" => "Hiển thị");
$config['static'][$nametype]['name'] = true;
$config['static'][$nametype]['content_cke'] = true;

/* Giới thiệu */
// $nametype = "slide-info";
// $config['static'][$nametype]['title_main'] = "Thông tin slide";
// $config['static'][$nametype]['check'] = array("hienthi" => "Hiển thị");
// $config['static'][$nametype]['images'] = false;
// $config['static'][$nametype]['icon'] = false;
// $config['static'][$nametype]['file'] = false;
// $config['static'][$nametype]['name'] = false;
// $config['static'][$nametype]['nametop'] = true;
// $config['static'][$nametype]['namebot'] = true;
// $config['static'][$nametype]['desc'] = true;
// $config['static'][$nametype]['desc_cke'] = false;
// $config['static'][$nametype]['content'] = false;
// $config['static'][$nametype]['content_cke'] = true;
// $config['static'][$nametype]['seo'] = false;
// $config['static'][$nametype]['width'] = 500;
// $config['static'][$nametype]['height'] = 335;
// $config['static'][$nametype]['width_icon'] = 500;
// $config['static'][$nametype]['height_icon'] = 335;
// $config['static'][$nametype]['img_type'] = '.jpg|.gif|.png|.jpeg|.gif';
// $config['static'][$nametype]['img_type_icon'] = '.jpg|.gif|.png|.jpeg|.gif';
// $config['static'][$nametype]['file_type'] = '.doc|.docx|.pdf|.rar|.zip|.ppt|.pptx|.xls|.xlsx|.jpg|.png|.gif';
