...
class Functions
{
    ...
    if (!empty($id)) {
      ...
      if ($enable_gallery && !empty($data['deleted_images'])) {
        foreach (explode('|', $data['deleted_images']) as $gid) {
          $gid = (int)$gid;
          if ($gid > 0) {
            $gallery = $this->d->rawQueryOne("SELECT `file` FROM tbl_gallery WHERE id = ?", [$gid]);
            if ($gallery && !empty($gallery['file'])) {
              @unlink(UPLOADS . $gallery['file']);
            }
            $this->d->execute("DELETE FROM tbl_gallery WHERE id = ?", [$gid]);
          }
        }
      }
      if ($enable_gallery && !empty($data['id-filer'])) {
        foreach ($data['id-filer'] as $k => $gid) {
          ...
          if ($gid > 0) {
            $this->d->execute("UPDATE `tbl_gallery` SET numb = ?, name = ? WHERE id = ?", [$numb, $name, $gid]);
          }
        }
      }
  ...
  public function delete_data(array $options = []): void
  {
    ...
    $row = $this->d->rawQueryOne("SELECT file FROM `$table` WHERE id = ?", [$id]);
    if (!$row) {
    ...
    if ($table === 'tbl_gallery') {
      ...
      $deleted = $this->d->execute("DELETE FROM tbl_gallery WHERE id = ?", [$id]);
    ...
    if ($delete_seo) {
      if ($type) {
        $this->d->execute("DELETE FROM tbl_seo WHERE id_parent = ? AND `type` = ?", [$id, $type]);
      } else {
        $this->d->execute("DELETE FROM tbl_seo WHERE id_parent = ?", [$id]);
      }
    }
    $deleted = $this->d->execute("DELETE FROM `$table` WHERE id = ?", [$id]);
    $this->transfer($deleted ? xoadulieuthanhcong : xoadulieubiloi, $redirect, $deleted);
  }
  public function deleteMultiple_data(array $options = [])
  {
    ...
    $delete_result = $this->d->execute("DELETE FROM `$table` WHERE id IN ($placeholders)", $ids);
  ...
  // {
  //   $_SESSION['transfer_data'] = ['msg' => $msg, 'com' => $com, 'numb' => $numb];
  //   define('IS_TRANSFER', true);
  //   header("Location: index.php?com=transfer");
  //   exit();
  // }
  public function transfer($msg = '', $com = '', $numb = true)
  {
  ...
  public function checkSlug(array $data = []): string|false
  {
    ...
    foreach ($tablesToCheck as $tbl) {
      $tblEsc = mysqli_real_escape_string($this->d->link, $tbl);
      $slugColEsc = mysqli_real_escape_string($this->d->link, $slugCol);
      $checkTable = $this->d->rawQueryOne("SHOW TABLES LIKE '$tblEsc'");
      if (!$checkTable) continue;
      $checkCol = $this->d->rawQueryOne("SHOW COLUMNS FROM `$tblEsc` LIKE '$slugColEsc'");
      ...
      $sql .= " LIMIT 1";
      $exist = $this->d->rawQueryOne($sql, $params);
      if ($exist) {
        return $errorMsg;
      }
    }
  ...
  {
    $row = $this->d->rawQueryOne("SELECT file, options FROM tbl_photo WHERE type = 'watermark' LIMIT 1");
  ...
  public function createThumb(
    ...
    if ($add_watermark && method_exists($this, 'addWatermark')) {
      $wm_data = $this->d->rawQueryOne("SELECT file, options, date_updated, status FROM `tbl_photo` WHERE type = 'watermark'LIMIT 1");
      if (!empty($wm_data)) {
  ...
  {
    $rows = $this->d->rawQuery("SELECT * FROM `tbl_order_status` ORDER BY id");
  ...
  {
    $rows = $this->d->rawQuery("SELECT * FROM `tbl_news` WHERE type = ? ORDER BY numb, id DESC", ["hinh-thuc-thanh-toan"]);
