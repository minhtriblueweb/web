<?php
$slides = $func->show_data([
  'table'  => 'tbl_photo',
  'type'  => 'slideshow',
  'status' => 'hienthi',
  'select' => "file, name$lang, link, options"
]);
$tintuc = $func->show_data([
  'table' => 'tbl_news',
  'status' => 'hienthi,noibat',
  'type'   => 'tin-tuc',
  'select' => "file, name$lang, desc$lang, slug$lang,date_updated",
  'limit'  => 8
]);
$feedback = $func->show_data([
  'table' => 'tbl_news',
  'status' => 'hienthi',
  'type'   => 'danh-gia',
  'select' => "file, name$lang, desc$lang,content$lang"
]);
$banchay = $func->show_data([
  'table'  => 'tbl_product',
  'status' => 'hienthi,banchay',
  'select' => "id, file, slug$lang, name$lang, sale_price, regular_price, views,discount",
  'limit' => 10
]);
$brand = $func->show_data([
  'table'  => 'tbl_product_brand',
  'status' => 'hienthi,noibat',
  'select' => "id,slug$lang,name$lang,file"
]);
$productList = $func->show_data([
  'table'  => 'tbl_product_list',
  'status' => 'hienthi,noibat',
  'select' => "id, slug$lang, name$lang"
]);
$background_bc = $d->rawQueryOne("SELECT `file` FROM tbl_photo WHERE type = ? AND FIND_IN_SET('hienthi', status) LIMIT 1", ['background_bc'])['file'] ?? '';
// SEO
$seo_data = $d->rawQueryOne("SELECT * FROM tbl_seopage WHERE type = ?", ['trang-chu']);
...
$seo->set('photo', !empty($seo_data['file']) ? $func->getImageCustom(['file' => $seo_data['file'], 'width' => 600, 'height' => 315, 'zc' => 2, 'src_only' => true]) : '');
